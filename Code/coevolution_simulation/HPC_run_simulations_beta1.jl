...
@everywhere function run_simulations_on_HPC(m)
    ...
    folder =  string(current_directory,"/../../Results/beta_1/")
    # Define folder where networks are stored
    # It currently is set to _20_20, which means it uses the data of networks consisting of 40 species, with 20 resource and 20 consumer species. If you want to run model with networks of different sizes or resource-consumer ratio, change numbers.
       ...
       mkdir(replace(net_files[i],".csv"=>""))
	    ...
	    # Make sure no values are negative
   	    ...
   	    # Make sure no values are negative
            ...
            theta = rand(theta_distribution,n_sp) * 10
            # Sample initial trait values
            init = rand(theta_distribution,n_sp) * 10
