...
measure_module_connectance = function(connectance_og,replica){
 ...
 # Load required libraries
  ...
  membership = read.table(paste('Data/for_modular/resultsSA/MEMBERS_C_',connectance_og,'_R_',replica,'.txt',sep =''))
  ...
  # Load network file
  network = read.csv(paste('Data/networks/C_',connectance_og,'_R_',replica,'.csv',sep = ''),header=FALSE)

  # Name rows and columns of dataframe
  colnames(network) = c(paste('C',1:30,sep=''))
  rownames(network) = c(paste('R',1:30,sep=''))
...
measure_module_metrics = function(connectance_from,connectance_to,connectance_by,replica_from,replica_to,replica_by){
  ...
  # Write data to csv
  write.csv(data_frame_analysis,paste("Results/processed_output/module_metrics/module_metrics.csv",sep=""))
}
